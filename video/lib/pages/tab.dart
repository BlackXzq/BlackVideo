...
class _TabPageState extends State<TabPage> {
  ...
  final _navigatorKeys = BLTabBar.tabs
      .map<GlobalKey<NavigatorState>>((v)=> GlobalKey<NavigatorState>()).toList();
  final _focusScopeNodes = BLTabBar.tabs.map<FocusScopeNode>((v) => FocusScopeNode()).toList();
  ...
  Widget build(BuildContext context) {
    return WillPopScope(
      ...
      child: IndexedStack(
        index: _tabIndex,
        children: BLTabBar.tabs
            .asMap()
            .entries
            .map<Widget>((entry) => FocusScope(
