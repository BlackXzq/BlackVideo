...
class WgFactory {
  ...
  Logger getLogger(String name) {
    if (_loggers[name] == null) {
      Logger.root.level = BLConfig.loggerLevel;
  ...
  Persistor<AppState> getPersistor() {
    if (_persistor == null) {
      _persistor = Persistor<AppState>(
        storage: FlutterStorage(BLConfig.packageInfo.packageName),
        decoder: (json) {
          var state = AppState.fromJson(json);
          if (compareVersion(state.version, BLConfig.packageInfo.version, 2) !=
              0) {
            state = AppState();
          }
          return state;
